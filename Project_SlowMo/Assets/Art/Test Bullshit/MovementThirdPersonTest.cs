...
public class MovementThirdPersonTest : MonoBehaviour
{
    ...
    [SerializeField]
    private CinemachineFreeLook neutralCam = null;
    [SerializeField]
    ...
    private void Start()
    {
        ...
        anim = GetComponent<Animator>();
        animatorScript = GetComponentInChildren<PlayerAnimatorScript>();
    ...
    private void Update()
    {
        if (isAcceptingInput)
        {
            ...
            if (!prone)
            {
                offset.transform.localPosition = new Vector3(0, 1.25f, 0);
            }
            else
            {
            ...
            if (Input.GetKeyDown(KeyCode.C) && !airTime)
            {
                timer = 0;
                held = false;
            }
            ...
            if (Input.GetKey(KeyCode.C) && !rolling && !airTime)
            {
                isPressing = true;
            ...
            if (Input.GetKeyUp(KeyCode.C) && held == false && !airTime)
            {
                if (!crouching && !prone && !rolling)
                {
            ...
            if (Input.GetKeyDown(KeyCode.LeftControl) && movedist > 1.4f && !sprinting && !airTime)
            {
            ...
            //======SHOOT DODGE=======
            if (Input.GetKeyDown(KeyCode.Space) && !airTime && Time.timeScale > 0 && isAllowedToBD)
            {
                if (!rolling && !prone)
                {
                    MoveSpeed = 5f;
    ...
    void FixedUpdate()
    {
        ...
        if (animatorScript.curAimPercent > 0.0f || direction.magnitude > 0)
        {
